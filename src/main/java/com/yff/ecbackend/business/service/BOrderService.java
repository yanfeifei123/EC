...
//                System.out.println(stime+"  "+etime);
    ...
    public OrderSummary findByOrderSummaryTodayyesterday(String branchid, String datetime) {
        ...
        dataSql.append(" sum(if(a.`status`=1,  a.totalfee,0 )) totalfee, count(if(a.`status`=1 , a.branchid,null)) ordernum,");
    ...
    public OrderSummary findByOrderSummaryRange(String branchid, String startdate, String enddate) {
        StringBuilder dataSql = new StringBuilder();
        dataSql.append("SELECT sum(if(a.`status`=1,  a.totalfee,0 )) totalfee, count(if(a.`status`=1 , a.branchid,null)) ordernum, sum(if(a.`status`=2,  a.totalfee,0 ))  refundamount,count(if(a.`status`=2 , a.branchid,null) ) refundformnum FROM u_order a where a.branchid=:branchid");
